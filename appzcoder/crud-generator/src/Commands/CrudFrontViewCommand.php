...
class CrudFrontViewCommand extends Command
{
    /**
     ...
     * @var string
     */
    protected $signature = 'crud:frontview
    ...
    public function handle()
    {
        ...
        $this->moduleName = strtolower($this->option('module-name'));
        $this->crudName = str_replace(' ','',strtolower($this->argument('name')));
        ...
        $this->modelNamePlural = $this->argument('name');
        $this->modelName = str_singular($this->argument('name'));
        $this->modelNameCap = ucfirst($this->modelName);
        $this->modelNameSmall = strtolower(str_replace(' ','-',$this->modelName));
        $this->customData = $this->option('custom-data');
        $this->viewUrlName = $this->option('viewUrlName');
        ...
        $this->viewName = snake_case(str_replace(' ','',$this->argument('name')), '-');

        $viewDirectory = config('view.paths')[0] . '/';
        if ($this->option('view-path')) {
            $this->userViewPath = $this->option('view-path');
            $path = $viewDirectory . $this->userViewPath . '/' . str_singular($this->viewName) . '/';
        } else {
            $path = $viewDirectory . str_singular($this->viewName) . '/';
        }

        //echo $path; exit;

        $this->viewTemplateDir = isset($this->userViewPath)
            ? strtolower($this->moduleName) . '::' . str_singular($this->viewName)
            : str_singular($this->viewName);
        ...
        foreach ($this->formFields as $key => $value) {
            ...
            if($value['required']==1){
                $this->formBodyHtml .= '{{ $item->' . $field . ' }}</br>';
            }
    ...
    {
        return [
            'view' => ['formHeadingHtml', 'formBodyHtml', 'crudName', 'viewUrlName', 'crudNameCap', 'modelName','modelNameSmall', 'viewName', 'routeGroup', 'primaryKey'],
            'form' => ['formFieldsHtml'],
            'create' => ['crudName', 'crudNameCap', 'modelName', 'modelNameSmall','viewUrlName', 'modelNameCap', 'viewName', 'routeGroup', 'viewTemplateDir'],
            'edit' => ['crudName', 'crudNameSingular', 'crudNameCap', 'modelNameSmall','viewUrlName', 'modelNameCap', 'modelName', 'viewName', 'routeGroup', 'primaryKey', 'viewTemplateDir'],
            'detail' => ['formHeadingHtml', 'formBodyHtml', 'modelNameSmall','viewUrlName', 'formBodyHtmlForShowView', 'crudName', 'crudNameSingular', 'crudNameCap', 'modelName', 'viewName', 'routeGroup', 'primaryKey'],
        ];
    }
    ...
    {
        $dynamicViewTemplate = config('crudgenerator.frontend_dynamic_view_template')
            ? config('crudgenerator.frontend_dynamic_view_template')
    ...
    protected function createSelectField($item)
    {
        ...
        if($item['options']!='db'){
            ...
            $markup = File::get($this->viewDirectoryPath . 'form-fields/select-db-field.blade.stub');
            $markup = str_replace($start . 'options' . $end, $item['name'].'s', $markup);
        }
        $markup = str_replace($start . 'required' . $end, $required, $markup);
        $markup = str_replace($start . 'itemName' . $end, $item['name'], $markup);
        $markup = str_replace($start . 'crudNameSingular' . $end, $this->crudNameSingular, $markup);
