...
(function($){$.widget("ui.scrollAppend", {
	...
	setSortBy: function (sort_by) {
		var self = this;
		self.options.params['sort_by'] = sort_by;
	},
		...
		$.ajax({
			...
			success: function(html){
				...
				if(html.last_page < self.page) {
					...
					//console.log(data);
					if(self.page==1)
					$(self.options.appendTo).html(tmpl('tmpl-item-grid', data));
					else
					$(self.options.appendTo).append(tmpl('tmpl-item-grid', data));
