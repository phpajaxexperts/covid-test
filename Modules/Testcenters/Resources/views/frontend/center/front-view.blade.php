...
<script type="text/x-tmpl" id="tmpl-item-grid">
    ...
    <div class="col-sm-3 p-2">
        ...
        <div class="text-center mt-2">
            <a href="{{ url('/centers/detail/{%=o.items[i].ID%}') }}" title="{%=o.items[i].title%}">
                {%=o.items[i].title%}
            </a>
        </div>
    </div>
        ...
        <div class="row" id="divItems">
            @isset($centers)
                @foreach($centers as $item)
                        ...
                        <div class="col-sm-3 p-2">
                            <div class="item">
                                <a href="{{ url('/center/detail/' . $item->ID) }}" title="Center">
                            ...
                            <div class="text-center mt-2">
                                <a href="{{ url('/centers/detail/' . $item->ID) }}" title="centers">
                                {{ $item->title }}
                                </a>
                            </div>
                        </div>
